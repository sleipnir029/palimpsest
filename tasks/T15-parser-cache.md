...
- File `src/palimpsest/cache.py` (replacing the empty stub) exports:
  - DDL applied on first init (in addition to T05's tables):
    ...
    - `parser_runs(paper_sha256, parser_name CHECK IN ('docling','mineru','olmocr','chandra'), parser_ver, parsed_at, output_path, gpu_seconds, gpu_cost_eur, run_id, PRIMARY KEY (paper_sha256, parser_name, parser_ver))`
  - Class `ParserCache`:
    ...
    - `def has_all_parsers(self, sha256: str) -> bool` — true iff 4 rows exist for the sha across all 4 parser names.
    ...
    - `def list_unseen(self, pdfs: list[Path]) -> list[tuple[Path, str]]` — returns `[(path, sha256), ...]` for PDFs missing all four parser outputs.
- File `tests/test_cache.py` covers:
  - Insert paper, insert 4 parser runs, `has_all_parsers` returns True.
