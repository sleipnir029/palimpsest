...
#   ./build.sh docling          # build palimpsest/docling:0.2.2 locally
#   ./build.sh mineru push      # build palimpsest/mineru:0.2.2 and push
...
# defaults to docker.io/<your-user>/palimpsest-<parser>:0.2.2 via DOCKERHUB_USERNAME.
#   DOCKERHUB_USERNAME=<you> ./build.sh mineru push
set -euo pipefail

PARSER="${1:?usage: ./build.sh <docling|mineru|chandra|dots|paddle> [push]}"
TAG="palimpsest/${PARSER}:0.2.2"     # local build label
...
if [[ "${2:-}" == "push" ]]; then
    PUSH_TAG="${PUSH_TAG:-docker.io/${DOCKERHUB_USERNAME:?set DOCKERHUB_USERNAME or PUSH_TAG}/palimpsest-${PARSER}:0.2.2}"
