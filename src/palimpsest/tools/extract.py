...
def _build_system_prompt(skill_body: str, jsonschema_str: str, norm: str) -> str:
    ...
    return (
        ...
        "- `evidence` MUST carry `page` (int) and the four per-corner bbox "
        "floats `bbox_x0`, `bbox_y0`, `bbox_x1`, `bbox_y1` (page-relative; F4) "
        "located in the parsed content. Do NOT emit `paper.sha256` or "
        "`parser_name` — the runtime injects both after parsing. Omit a "
        "measurement entirely rather than ship it without page + all 4 bbox slots.\n"
