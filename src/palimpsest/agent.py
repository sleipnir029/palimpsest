...
def build_system_prompt(cost_meter) -> str:
    ...
    return (
        ...
        "the engine and its fixtures. bash is OS-sandboxed to the workspace by default (its "
        "writes can't escape it); still keep its work inside the workspace and never use it to "
        "touch the engine, the graph store, or the ledger.\n"
